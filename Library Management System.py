...
class Library:
        ...
        if title.strip() == "" or author.strip() == "" or releasedate.strip() == "" or page.strip() == "":
            ...
            if releasedate.isnumeric() and page.isnumeric():
                ...
                with open(self.database_path, 'a') as file:
                    file.write(f"{title},{author},{releasedate},{page},available,{date.strftime("%d/%m/%Y %H:%M:%S")}\n")
                print(f"{title} by {author} {releasedate} {page} {date.strftime("%d/%m/%Y %H:%M:%S")} has been added to the library.")
            else:
    ...
    def list_books(self):
        with open(self.database_path, 'r') as file:
            for line in file:
                title, author, releasedate, page, status, date = line.strip().split(',')
                print(f"Title: {title}, Author: {author}, Release Date: {releasedate}, Pages: {page}, Status: {status}, Addition Date: {date}")
    ...
    def check_in_book(self, title):
        ...
        found = False
        for line in lines:
            current_title, author, releasedate, page, status, date = line.strip().split(',')
